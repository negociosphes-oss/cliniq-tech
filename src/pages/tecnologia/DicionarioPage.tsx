...
import { Plus, Layers, Trash2, Save, X, Loader2, UploadCloud, Zap, ShieldCheck } from 'lucide-react';
...
export function DicionarioPage() {
  ...
  useEffect(() => { if (tenantId && tenantId > 0) fetchData(); }, [activeTab, tenantId]);

  const fetchData = async () => {
    setLoading(true);
    try {
      const queryFilter = `tenant_id.eq.${tenantId},tenant_id.eq.1`; // LÓGICA HÍBRIDA: Local + Global

      if (activeTab === 'tecnologias') {
        const { data } = await supabase.from('dict_tecnologias').select('*').or(queryFilter).order('nome');
        setTecnologias(data || []);
      } else if (activeTab === 'fabricantes') {
        const { data } = await supabase.from('dict_fabricantes').select('*').or(queryFilter).order('nome');
        setFabricantes(data || []);
      } else {
        const { data } = await supabase.from('dict_modelos').select('*, tecnologia:dict_tecnologias(nome), fabricante:dict_fabricantes(nome)').or(queryFilter).order('nome');
        setModelos(data || []);
        
        const [tec, fab] = await Promise.all([
            supabase.from('dict_tecnologias').select('*').or(queryFilter).order('nome'),
            supabase.from('dict_fabricantes').select('*').or(queryFilter).order('nome')
        ]);
  ...
  const handleSaveSingle = async () => {
    ...
    try {
      if (activeTab === 'tecnologias') {
        ...
        await supabase.from('dict_tecnologias').insert([{ nome: simpleName, tenant_id: tenantId }]);
      } else if (activeTab === 'fabricantes') {
        if(!simpleName) return alert('Digite o nome do Fabricante.');
        await supabase.from('dict_fabricantes').insert([{ nome: simpleName, tenant_id: tenantId }]);
      } else {
        ...
        await supabase.from('dict_modelos').insert([{ ...modeloForm, tenant_id: tenantId }]);
      }
  ...
  const handleSaveBulk = async () => {
    ...
    try {
      const payload = items.map(nome => ({ nome, tenant_id: tenantId }));
      if (activeTab === 'tecnologias') {
  ...
  const handleDelete = async (id: number, itemTenantId: number) => {
    if (itemTenantId === 1 && tenantId !== 1) return alert("Você não pode excluir um item do Dicionário Base da plataforma.");
  ...
  if (!tenantId) return <div className="p-12 flex justify-center"><Loader2 className="animate-spin text-indigo-600" size={40}/></div>;
               ...
               <tbody className="divide-y divide-slate-100 text-sm font-medium">
                  {activeTab === 'tecnologias' && tecnologias.map(t => (
                     <tr key={t.id} className="hover:bg-slate-50">
                         <td className="p-5 pl-8 flex items-center gap-2">
                             {t.tenant_id === 1 && tenantId !== 1 && <span title="Modelo Base Global"><ShieldCheck size={14} className="text-indigo-500"/></span>} 
                             {t.nome}
                         </td>
                         <td className="p-5 text-right pr-8"><button onClick={() => handleDelete(t.id, t.tenant_id)} className={`transition-colors ${t.tenant_id === 1 && tenantId !== 1 ? 'text-slate-300 cursor-not-allowed' : 'text-rose-500 hover:text-rose-700'}`}><Trash2 size={18}/></button></td>
                     </tr>
                  ))}
                  {activeTab === 'fabricantes' && fabricantes.map(f => (
                     <tr key={f.id} className="hover:bg-slate-50">
                         <td className="p-5 pl-8 flex items-center gap-2">
                             {f.tenant_id === 1 && tenantId !== 1 && <span title="Modelo Base Global"><ShieldCheck size={14} className="text-indigo-500"/></span>} 
                             {f.nome}
                         </td>
                         <td className="p-5 text-right pr-8"><button onClick={() => handleDelete(f.id, f.tenant_id)} className={`transition-colors ${f.tenant_id === 1 && tenantId !== 1 ? 'text-slate-300 cursor-not-allowed' : 'text-rose-500 hover:text-rose-700'}`}><Trash2 size={18}/></button></td>
                     </tr>
                  ))}
                     ...
                     <tr key={m.id} className="hover:bg-slate-50">
                         <td className="p-5 pl-8">
                             <div className="font-bold text-slate-700 flex items-center gap-2">
                                 {m.tenant_id === 1 && tenantId !== 1 && <span title="Modelo Base Global"><ShieldCheck size={12} className="text-indigo-500"/></span>} 
                                 {m.tecnologia?.nome}
                             </div>
                         ...
                         <td className="p-5 text-right pr-8"><button onClick={() => handleDelete(m.id, m.tenant_id)} className={`transition-colors ${m.tenant_id === 1 && tenantId !== 1 ? 'text-slate-300 cursor-not-allowed' : 'text-rose-500 hover:text-rose-700'}`}><Trash2 size={18}/></button></td>
                     </tr>
                  ))}
               </tbody>
             </table>
