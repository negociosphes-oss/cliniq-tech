...
import { Plus, Layers, Factory, Server, Trash2, Save, X, Loader2, UploadCloud, ShieldAlert, Zap } from 'lucide-react';
...
export function DicionarioPage() {
  ...
  const [simpleName, setSimpleName] = useState(''); // Para Tec e Fab
  ...
  const fetchData = async () => {
    ...
    try {
      if (activeTab === 'tecnologias') {
        ...
        setModelos(data || []);
        // Precisa carregar tec e fab para os selects
  ...
  const handleSaveSingle = async () => {
    ...
    try {
      if (activeTab === 'tecnologias') {
        if(!simpleName) return alert('Digite o nome.');
        await supabase.from('dict_tecnologias').insert([{ nome: simpleName }]);
      } else if (activeTab === 'fabricantes') {
        if(!simpleName) return alert('Digite o nome.');
        await supabase.from('dict_fabricantes').insert([{ nome: simpleName }]);
      } else {
        if(!modeloForm.nome || !modeloForm.tecnologia_id || !modeloForm.fabricante_id) return alert('Preencha os campos obrigatórios.');
  ...
  const handleDelete = async (id: number) => {
    ...
    try {
        ...
        alert('Não é possível excluir. Este item já está sendo usado por um Modelo ou Equipamento.');
    }
  };
    ...
    <div className="p-8 animate-fadeIn max-w-7xl mx-auto pb-24">
      <div className="flex flex-col md:flex-row justify-between items-end mb-8 gap-4">
        <div>
          ...
          <p className="text-slate-500 font-medium mt-2">Construa a base hierárquica do seu sistema.</p>
        </div>
      ...
      <div className="flex gap-4 mb-6 border-b border-slate-200">
        <button onClick={() => setActiveTab('tecnologias')} className={`py-4 px-6 text-sm font-black uppercase tracking-widest border-b-4 transition-all ${activeTab === 'tecnologias' ? 'border-indigo-600 text-indigo-700' : 'border-transparent text-slate-400 hover:text-slate-700'}`}>1. Tecnologias</button>
      ...
      {isSingleModalOpen && (
         <div className="fixed inset-0 bg-slate-900/80 backdrop-blur-sm z-[2000] flex items-center justify-center p-4">
             <div className="bg-white rounded-3xl p-6 w-full max-w-lg shadow-2xl">
                 ...
                 {activeTab !== 'modelos' ? (
                     <div>
                         <label className="text-xs font-bold text-slate-500 uppercase">Nome Padrão</label>
                         <input className="w-full h-12 px-4 border rounded-xl mt-2" value={simpleName} onChange={e => setSimpleName(e.target.value)}/>
                     </div>
                 ) : (
                     <div className="space-y-4">
                         <div>
                            <label className="text-xs font-bold text-slate-500 uppercase">1. Tecnologia</label>
                            <select className="w-full h-12 px-4 border rounded-xl mt-1" value={modeloForm.tecnologia_id} onChange={e => setModeloForm({...modeloForm, tecnologia_id: e.target.value})}>
                                <option value="">Selecione...</option>{tecnologias.map(t => <option key={t.id} value={t.id}>{t.nome}</option>)}
                            </select>
                         </div>
                         <div>
                            <label className="text-xs font-bold text-slate-500 uppercase">2. Fabricante</label>
                            <select className="w-full h-12 px-4 border rounded-xl mt-1" value={modeloForm.fabricante_id} onChange={e => setModeloForm({...modeloForm, fabricante_id: e.target.value})}>
                                <option value="">Selecione...</option>{fabricantes.map(f => <option key={f.id} value={f.id}>{f.nome}</option>)}
                            </select>
                         </div>
                         <div>
                            <label className="text-xs font-bold text-slate-500 uppercase">3. Nome do Modelo</label>
                            <input className="w-full h-12 px-4 border rounded-xl mt-1" value={modeloForm.nome} onChange={e => setModeloForm({...modeloForm, nome: e.target.value})}/>
                         </div>
                         <div className="grid grid-cols-2 gap-4 pt-2 border-t">
                            <div>
                                <label className="text-xs font-bold text-slate-500 uppercase">Proteção Elétrica</label>
                                <select className="w-full h-10 px-3 border rounded-lg mt-1" value={modeloForm.classe_protecao_eletrica} onChange={e => setModeloForm({...modeloForm, classe_protecao_eletrica: e.target.value})}><option>Classe I</option><option>Classe II</option><option>Energizado Internamente</option></select>
                            </div>
                            <div>
                                <label className="text-xs font-bold text-slate-500 uppercase">Peça Aplicada</label>
                                <select className="w-full h-10 px-3 border rounded-lg mt-1" value={modeloForm.tipo_peca_aplicada} onChange={e => setModeloForm({...modeloForm, tipo_peca_aplicada: e.target.value})}><option>Não se aplica</option><option>Tipo B</option><option>Tipo BF</option><option>Tipo CF</option></select>
                            </div>
                         </div>
                     </div>
                 )}
                 ...
                 <div className="flex justify-end gap-3 mt-6">
                    <button onClick={() => setIsSingleModalOpen(false)} className="px-6 py-3 font-bold text-slate-500">Cancelar</button>
                    <button onClick={handleSaveSingle} disabled={saving} className="px-6 py-3 bg-indigo-600 text-white font-bold rounded-xl flex gap-2 items-center">{saving ? <Loader2 className="animate-spin" size={18}/> : <Save size={18}/>} Salvar</button>
                 </div>
             </div>
         </div>
      )}
      ...
      {isBulkModalOpen && (
         <div className="fixed inset-0 bg-slate-900/80 backdrop-blur-sm z-[2000] flex items-center justify-center p-4">
             <div className="bg-white rounded-3xl p-6 w-full max-w-xl shadow-2xl">
                 ...
                 <p className="text-sm text-slate-500 mb-4">Copie uma coluna do Excel (Ctrl+C) e cole aqui (Ctrl+V). Cada linha será um novo {activeTab === 'tecnologias' ? 'Tipo de Equipamento' : 'Fabricante'}.</p>
                 ...
                 <textarea 
                    ...
                    placeholder={`Monitor Multiparâmetro\nVentilador Pulmonar\nBomba de Infusão...`}
                    value={bulkText}
                 ...
                 <div className="flex justify-end gap-3 mt-6">
                    ...
                    <button onClick={handleSaveBulk} disabled={saving} className="px-6 py-3 bg-emerald-600 text-white font-bold rounded-xl flex gap-2 items-center shadow-lg hover:bg-emerald-700">{saving ? <Loader2 className="animate-spin" size={18}/> : <Save size={18}/>} Importar Tudo</button>
                 </div>
             </div>
         </div>
      )}
