...
import { Users, Plus, Edit, Trash2, Shield, Mail, Key, Briefcase, X, Loader2 } from 'lucide-react';
...
export function UsuariosList() {
    ...
    const [formData, setFormData] = useState({
        nome: '', email: '', senha: '', cargo: '', nivel_acesso: 'tecnico'
    });

    // 🚀 CORREÇÃO: Busca Direta e Blindada
    useEffect(() => {
        fetchUsuarios();
    }, []);
    ...
    const fetchUsuarios = async () => {
        setLoading(true);
        try {
            // Busca todos os usuários sem depender de URL. Se precisar de Tenant, passe por Props no futuro.
            ...
            setLoading(false); // 🚀 Isso garante que a tela destrave!
        }
    };
    ...
    const openModal = (user: any = null) => {
        if (user) {
            ...
            setFormData({
                ...
                cargo: user.cargo || '', nivel_acesso: user.nivel_acesso || 'tecnico'
            });
        } else {
            setEditingId(null);
    ...
    const handleSave = async (e: React.FormEvent) => {
        e.preventDefault();
        setSaving(true);
        // Por padrão, se não tem tenant na URL, salva como 1 (Master)
        ...
        <div className="animate-fadeIn space-y-6">
            <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 bg-theme-card p-6 rounded-3xl border border-theme shadow-sm relative overflow-hidden">
                ...
                <div className="flex gap-3 relative z-10 w-full sm:w-auto">
                    ...
                    <button onClick={() => openModal()} className="bg-primary-theme text-white px-5 py-2.5 rounded-xl text-sm font-black flex items-center justify-center gap-2 hover:opacity-90 transition-all shadow-lg active:scale-95 flex-1 sm:flex-none">
                                    ...
                                    <tr key={user.id} className="hover:bg-theme-page/30 transition-colors group">
                                        ...
                                        <td className="p-4 pr-6 text-right">
                                            <div className="flex items-center justify-end gap-1 opacity-0 group-hover:opacity-100 transition-opacity">
                                                <button onClick={() => openModal(user)} className="p-2 text-theme-muted hover:text-blue-500 hover:bg-blue-500/10 rounded-xl transition-all" title="Editar"><Edit size={18} /></button>
                                                <button onClick={() => handleDelete(user.id, user.nome)} className="p-2 text-theme-muted hover:text-rose-500 hover:bg-rose-500/10 rounded-xl transition-all" title="Remover Acesso"><Trash2 size={18} /></button>
                                            </div>
                                        </td>
                                    </tr>
                                ))
                    ...
                    <div className="bg-theme-card w-full max-w-xl rounded-[2rem] shadow-2xl border border-theme overflow-hidden animate-slideUp">
                        <div className="p-6 border-b border-theme flex items-center justify-between bg-theme-page/50">
                            <h3 className="text-xl font-black text-theme-main flex items-center gap-2">
                                <Users className="text-primary-theme" size={24}/> {editingId ? 'Editar Credenciais' : 'Convidar Novo Usuário'}
                            </h3>
                            ...
                            <div className="pt-2">
                                <div className="bg-theme-page border border-theme rounded-2xl p-5 relative overflow-hidden">
                                    ...
                                    <select 
                                        ...
                                        value={formData.nivel_acesso} onChange={e => setFormData({...formData, nivel_acesso: e.target.value})}
                                    >
