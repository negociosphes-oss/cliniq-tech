...
export function OSPublicView() {
  ...
  // 1. CORREÇÃO DAS IMAGENS: Agora com verificação de segurança
  ...
  // 2. TRATAMENTO DE FOTOS SEGURO: Evita o erro startsWith
  let fotos: string[] = [];
  try {
    const anexosRaw = os.anexos;
    if (anexosRaw) {
        if (Array.isArray(anexosRaw)) {
            fotos = anexosRaw.filter(f => typeof f === 'string');
        } else if (typeof anexosRaw === 'string') {
            if (anexosRaw.startsWith('[')) {
                const parsed = JSON.parse(anexosRaw);
  ...
  return (
    <div className="min-h-screen bg-[#F2F2F7] dark:bg-black font-sans text-slate-900 dark:text-white">
            ...
            <div className="pt-8 border-t border-black/5 dark:border-white/5 space-y-6">
                ...
                <div className="grid md:grid-cols-2 gap-4">
                   ...
                   <div className="bg-amber-500/10 p-5 rounded-2xl">
                      <p className="text-[10px] font-black uppercase tracking-widest text-amber-600 mb-2 flex items-center gap-1"><Activity size={12}/> Causa Raiz</p>
      ...
      {/* 2. VISÃO DO PDF OFICIAL (A4) - OCULTA NA TELA */}
      <div className="hidden print:block bg-white text-black font-sans p-8 w-full max-w-[210mm] mx-auto text-[11px]">
             ...
             <table className="w-full border-collapse border border-black text-[11px]">
                <tbody>
                   <tr>
                      ...
                      <td className="border border-black p-1 w-[35%]">{os.equipamentos?.clientes?.nome || 'N/A'}</td>
          ...
          <div className="mb-4">
             ...
             <div className="border border-black p-2 space-y-3">
                <div><span className="font-bold text-[10px] uppercase">Queixa:</span><br/>{os.descricao_problema || os.descricao || 'N/A'}</div>
                <div><span className="font-bold text-[10px] uppercase">Falha Constatada:</span><br/>{os.falha_constatada || 'N/A'}</div>
                <div><span className="font-bold text-[10px] uppercase">Solução Técnica:</span><br/>{os.solucao_aplicada || 'N/A'}</div>
             </div>
          </div>
          ...
          {fotos.length > 0 && (
            <div className="mb-4" style={{ pageBreakInside: 'avoid' }}>
               <div className="bg-slate-200 font-bold p-1 uppercase text-[10px] border border-black border-b-0">2. Evidências Fotográficas</div>
          ...
          <div className="mt-16 flex justify-between gap-8 px-8" style={{ pageBreakInside: 'avoid' }}>
              <div className="flex-1 border-t border-black text-center pt-2">
                  ...
                  <p className="text-[10px]">{os.tecnico_nome || 'Equipe Atlasum'}</p>
              </div>
              <div className="flex-1 border-t border-black text-center pt-2">
                  <p className="font-bold uppercase text-[10px]">Aceite do Cliente</p>
              </div>
          </div>
      </div>
    </div>
