    ...
    <div className="space-y-6 animate-fadeIn pb-32 min-h-[80vh]">
      {!laudo ? (
        <div className="flex flex-col items-center justify-center py-16 text-center bg-white dark:bg-slate-800 rounded-3xl border-2 border-dashed border-slate-200">
          ...
          <button onClick={() => setIsModalOpen(true)} className="bg-indigo-600 text-white px-8 py-4 rounded-2xl font-black shadow-xl mt-6 hover:bg-indigo-700 transition-all flex items-center gap-2">Iniciar Protocolo Profissional</button>
        </div>
      ) : (
        <div className="bg-white dark:bg-slate-800 p-8 rounded-3xl border shadow-sm">
          <div className="flex justify-between items-center mb-8">
            <div className="flex items-center gap-4">
              ...
              <div>
                <h4 className="text-lg font-black dark:text-white uppercase">CERTIFICADO #{laudo.id}</h4>
          ...
          <div className="grid grid-cols-2 gap-4">
            <button onClick={() => setIsCertOpen(true)} className="flex items-center justify-center gap-2 bg-indigo-600 text-white py-4 rounded-2xl font-black uppercase text-xs shadow-xl transition-all">
               <Download size={18}/> Visualizar Certificado RBC
            </button>
            <button onClick={() => setIsModalOpen(true)} className="border-2 border-slate-200 text-slate-400 py-4 rounded-2xl font-black uppercase text-xs hover:bg-slate-50 transition-all">
               Refazer Calibração
            </button>
          </div>
        </div>
